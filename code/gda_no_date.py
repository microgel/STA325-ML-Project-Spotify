...
x = np.zeros([m,n])
y = np.zeros([m,1])
with open('complete_project_data_no_date.csv') as csv_file:
    ...
    for row in csv_reader:
        if line_count != 0:
            ...
            j = 0
            if opt_artistscore:
                x[line_count-1,j] = row[2]
                j += 1
            if opt_danceability:
                x[line_count-1,j] = row[3]
                j += 1
            if opt_energy:
