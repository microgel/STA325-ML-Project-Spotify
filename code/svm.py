import matplotlib.pyplot as plt
import numpy as np
import json
import csv

from sklearn import svm
from sklearn import tree

def add_intercept(x):
    new_x = np.zeros((x.shape[0], x.shape[1] + 1), dtype=x.dtype)
    new_x[:, 0] = 1
    new_x[:, 1:] = x

    return new_x

# Set features to use
opt_artistscore = True
opt_danceability = True
opt_energy = False
opt_key = False
opt_loudness = True
opt_mode = False
opt_speechiness = True
opt_acousticness = True
opt_instrumentalness = True
opt_liveness = False
opt_valence = True
opt_tempo = True

# Set information
m = 3221    # Number of examples

# Set features to consider
n = 0
if opt_artistscore:
    n +=1
if opt_danceability:
    n +=1
if opt_energy:
    n +=1
if opt_key:
    n +=1
if opt_loudness:
    n +=1
if opt_mode:
    n +=1
if opt_speechiness:
    n +=1
if opt_acousticness:
    n +=1
if opt_instrumentalness:
    n +=1
if opt_liveness:
    n +=1
if opt_valence:
    n +=1
if opt_tempo:
    n +=1

# Load dataset
artist_l = []
track_l = []
year_l = []
month_l = []
x = np.zeros([m,n])
y = np.zeros([m,1])
with open('complete_project_data.csv') as csv_file:
    csv_reader = csv.reader(csv_file, delimiter=',')
    line_count = 0
    for row in csv_reader:
        if line_count != 0:
            artist_l.append(row[0])
            track_l.append(row[1])
            year_l.append(row[2])
            month_l.append(row[3])
            j = 0
            if opt_artistscore:
                x[line_count-1,j] = row[4]
                j += 1
            if opt_danceability:
                x[line_count-1,j] = row[5]
                j += 1
            if opt_energy:
                x[line_count-1,j] = row[6]
                j += 1
            if opt_key:
                x[line_count-1,j] = row[7]
                j += 1
            if opt_loudness:
                x[line_count-1,j] = row[8]
                j += 1
            if opt_mode:
                x[line_count-1,j] = row[9]
                j += 1
            if opt_speechiness:
                x[line_count-1,j] = row[10]
                j += 1
            if opt_acousticness:
                x[line_count-1,j] = row[11]
                j += 1
            if opt_instrumentalness:
                x[line_count-1,j] = row[12]
                j += 1
            if opt_liveness:
                x[line_count-1,j] = row[13]
                j += 1
            if opt_valence:
                x[line_count-1,j] = row[14]
                j += 1
            if opt_tempo:
                x[line_count-1,j] = row[15]
                j += 1
            y[line_count-1] = row[16]
        line_count += 1

m_train = 2416
m_valid = m - m_train
x_train = np.zeros([m_train,n])
y_train = np.zeros([m_train,1])
x_valid = np.zeros([m_valid,n])
y_valid = np.zeros([m_valid,1])

for i in range(0,m_train):
    x_train[i,:] = x[i,:]
    y_train[i] = y[i]
for i in range(0,m_valid):
    x_valid[i,:] = x[i + m_train,:]
    y_valid[i] = y[i + m_train]

clf_tree = tree.DecisionTreeClassifier(max_depth=int(1.5*n))
theta = clf_tree.fit(x_train,y_train)
h_tree = clf_tree.predict(x_train)
h_split_t = np.zeros([m_train,1])
for i in range(0,m_train):
    h_split_t[i] = h_tree[i]
svm_accuracy_tree_train = np.mean(h_split_t == y_train)

h_tree = clf_tree.predict(x_valid)
h_split_v = np.zeros([m_valid,1])
for i in range(0,m_valid):
    h_split_v[i] = h_tree[i]
svm_accuracy_tree_valid = np.mean(h_split_v == y_valid)

print('theta_tree')
print(theta)
print('accuracy_tree_train')
print(svm_accuracy_tree_train)
print('accuracy_tree_valid')
print(svm_accuracy_tree_valid)

tn_t = 0
fn_t = 0
tp_t = 0
fp_t = 0
for i in range(0,m_train):
    if h_split_t[i] == 0 and y_train[i] == 0:
        tn_t += 1
    if h_split_t[i] == 0 and y_train[i] == 1:
        fn_t += 1
    if h_split_t[i] == 1 and y_train[i] == 0:
        fp_t += 1
    if h_split_t[i] == 1 and y_train[i] == 1:
        tp_t += 1
tree_precision_train = tp_t/(tp_t + fp_t)
tree_recall_train = tp_t/(tp_t + fn_t)

print('tn_t = ',tn_t)
print('fn_t = ',fn_t)
print('fp_t = ',fp_t)
print('tp_t = ',tp_t)

print('training precision')
print(tree_precision_train)
print('training recall')
print(tree_recall_train)

tn_v = 0
fn_v = 0
tp_v = 0
fp_v = 0
for i in range(0,m_valid):
    if h_split_v[i] == 0 and y_valid[i] == 0:
        tn_v += 1
    if h_split_v[i] == 0 and y_valid[i] == 1:
        fn_v += 1
    if h_split_v[i] == 1 and y_valid[i] == 0:
        fp_v += 1
    if h_split_v[i] == 1 and y_valid[i] == 1:
        tp_v += 1
tree_precision_valid = tp_v/(tp_v + fp_v)
tree_recall_valid = tp_v/(tp_v + fn_v)

print('tn_v = ',tn_v)
print('fn_v = ',fn_v)
print('fp_v = ',fp_v)
print('tp_v = ',tp_v)

print('validation precision')
print(tree_precision_valid)
print('validation recall')
print(tree_recall_valid)

x = add_intercept(x)
n = n + 1

# Split into training and validation
m_train = 2416
m_valid = m - m_train
x_train = np.zeros([m_train,n])
y_train = np.zeros([m_train,1])
x_valid = np.zeros([m_valid,n])
y_valid = np.zeros([m_valid,1])

for i in range(0,m_train):
    x_train[i,:] = x[i,:]
    y_train[i] = y[i]
for i in range(0,m_valid):
    x_valid[i,:] = x[i + m_train,:]
    y_valid[i] = y[i + m_train]

y_train = y_train.ravel()

clf_lin = svm.SVC(gamma='scale',kernel='linear')
theta = clf_lin.fit(x_train,y_train)
h_lin = clf_lin.predict(x_train)
h_split_t = np.zeros([m_train,1])
for i in range(0,m_train):
    h_split_t[i] = h_lin[i]
svm_accuracy_lin_train = np.mean(h_split_t == y_train)

h_lin = clf_lin.predict(x_valid)
h_split_v = np.zeros([m_valid,1])
for i in range(0,m_valid):
    h_split_v[i] = h_lin[i]
svm_accuracy_lin_valid = np.mean(h_split_v == y_valid)

print('theta_lin')
print(theta)
print('accuracy_lin_train')
print(svm_accuracy_lin_train)
print('accuracy_lin_valid')
print(svm_accuracy_lin_valid)

tn_t = 0
fn_t = 0
tp_t = 0
fp_t = 0
for i in range(0,m_train):
    if h_split_t[i] == 0 and y_train[i] == 0:
        tn_t += 1
    if h_split_t[i] == 0 and y_train[i] == 1:
        fn_t += 1
    if h_split_t[i] == 1 and y_train[i] == 0:
        fp_t += 1
    if h_split_t[i] == 1 and y_train[i] == 1:
        tp_t += 1
lin_precision_train = tp_t/(tp_t + fp_t)
lin_recall_train = tp_t/(tp_t + fn_t)

print('tn_t = ',tn_t)
print('fn_t = ',fn_t)
print('fp_t = ',fp_t)
print('tp_t = ',tp_t)

print('training precision')
print(lin_precision_train)
print('training recall')
print(lin_recall_train)

tn_v = 0
fn_v = 0
tp_v = 0
fp_v = 0
for i in range(0,m_valid):
    if h_split_v[i] == 0 and y_valid[i] == 0:
        tn_v += 1
    if h_split_v[i] == 0 and y_valid[i] == 1:
        fn_v += 1
    if h_split_v[i] == 1 and y_valid[i] == 0:
        fp_v += 1
    if h_split_v[i] == 1 and y_valid[i] == 1:
        tp_v += 1
lin_precision_valid = tp_v/(tp_v + fp_v)
lin_recall_valid = tp_v/(tp_v + fn_v)

print('tn_v = ',tn_v)
print('fn_v = ',fn_v)
print('fp_v = ',fp_v)
print('tp_v = ',tp_v)

print('validation precision')
print(lin_precision_valid)
print('validation recall')
print(lin_recall_valid)

clf_rbf = svm.SVC(gamma='scale',kernel='rbf')
theta = clf_rbf.fit(x_train,y_train)
h_rbf = clf_rbf.predict(x_train)
h_split_t = np.zeros([m_train,1])
for i in range(0,m_train):
    h_split_t[i] = h_rbf[i]
svm_accuracy_rbf_train = np.mean(h_split_t == y_train)

h_rbf = clf_rbf.predict(x_valid)
h_split_v = np.zeros([m_valid,1])
for i in range(0,m_valid):
    h_split_v[i] = h_rbf[i]
svm_accuracy_rbf_valid = np.mean(h_split_v == y_valid)

print('theta_rbf')
print(theta)
print('accuracy_rbf_train')
print(svm_accuracy_rbf_train)
print('accuracy_rbf_valid')
print(svm_accuracy_rbf_valid)

tn_t = 0
fn_t = 0
tp_t = 0
fp_t = 0
for i in range(0,m_train):
    if h_split_t[i] == 0 and y_train[i] == 0:
        tn_t += 1
    if h_split_t[i] == 0 and y_train[i] == 1:
        fn_t += 1
    if h_split_t[i] == 1 and y_train[i] == 0:
        fp_t += 1
    if h_split_t[i] == 1 and y_train[i] == 1:
        tp_t += 1
rbf_precision_train = tp_t/(tp_t + fp_t)
rbf_recall_train = tp_t/(tp_t + fn_t)

print('tn_t = ',tn_t)
print('fn_t = ',fn_t)
print('fp_t = ',fp_t)
print('tp_t = ',tp_t)

print('training precision')
print(rbf_precision_train)
print('training recall')
print(rbf_recall_train)

tn_v = 0
fn_v = 0
tp_v = 0
fp_v = 0
for i in range(0,m_valid):
    if h_split_v[i] == 0 and y_valid[i] == 0:
        tn_v += 1
    if h_split_v[i] == 0 and y_valid[i] == 1:
        fn_v += 1
    if h_split_v[i] == 1 and y_valid[i] == 0:
        fp_v += 1
    if h_split_v[i] == 1 and y_valid[i] == 1:
        tp_v += 1
rbf_precision_valid = tp_v/(tp_v + fp_v)
rbf_recall_valid = tp_v/(tp_v + fn_v)

print('tn_v = ',tn_v)
print('fn_v = ',fn_v)
print('fp_v = ',fp_v)
print('tp_v = ',tp_v)

print('validation precision')
print(rbf_precision_valid)
print('validation recall')
print(rbf_recall_valid)

clf_poly = svm.SVC(gamma='scale',kernel='poly')
theta = clf_poly.fit(x_train,y_train)
h_poly = clf_poly.predict(x_train)
h_split_t = np.zeros([m_train,1])
for i in range(0,m_train):
    h_split_t[i] = h_poly[i]
svm_accuracy_poly_train = np.mean(h_split_t == y_train)

h_poly = clf_poly.predict(x_valid)
h_split_v = np.zeros([m_valid,1])
for i in range(0,m_valid):
    h_split_v[i] = h_poly[i]
svm_accuracy_poly_valid = np.mean(h_split_v == y_valid)

print('theta_poly')
print(theta)
print('accuracy_poly_train')
print(svm_accuracy_poly_train)
print('accuracy_poly_valid')
print(svm_accuracy_poly_valid)

tn_t = 0
fn_t = 0
tp_t = 0
fp_t = 0
for i in range(0,m_train):
    if h_split_t[i] == 0 and y_train[i] == 0:
        tn_t += 1
    if h_split_t[i] == 0 and y_train[i] == 1:
        fn_t += 1
    if h_split_t[i] == 1 and y_train[i] == 0:
        fp_t += 1
    if h_split_t[i] == 1 and y_train[i] == 1:
        tp_t += 1
poly_precision_train = tp_t/(tp_t + fp_t)
poly_recall_train = tp_t/(tp_t + fn_t)

print('tn_v = ',tn_v)
print('fn_v = ',fn_v)
print('fp_v = ',fp_v)
print('tp_v = ',tp_v)

print('training precision')
print(poly_precision_train)
print('training recall')
print(poly_recall_train)

tn_v = 0
fn_v = 0
tp_v = 0
fp_v = 0
for i in range(0,m_valid):
    if h_split_v[i] == 0 and y_valid[i] == 0:
        tn_v += 1
    if h_split_v[i] == 0 and y_valid[i] == 1:
        fn_v += 1
    if h_split_v[i] == 1 and y_valid[i] == 0:
        fp_v += 1
    if h_split_v[i] == 1 and y_valid[i] == 1:
        tp_v += 1
poly_precision_valid = tp_v/(tp_v + fp_v)
poly_recall_valid = tp_v/(tp_v + fn_v)

print('tn_v = ',tn_v)
print('fn_v = ',fn_v)
print('fp_v = ',fp_v)
print('tp_v = ',tp_v)

print('validation precision')
print(poly_precision_valid)
print('validation recall')
print(poly_recall_valid)

#clf_sig = svm.SVC(gamma='scale',kernel='sigmoid')
#theta = clf_sig.fit(x_train,y_train)
#h_sig = clf_sig.predict(x_train)
#h_split_t = np.zeros([m_train])
#for i in range(0,m_train):
#    h_split_t[i] = h_sig[i]
#svm_accuracy_sig_train = np.mean(h_split_t == y_train)

#h_sig = clf_sig.predict(x_valid)
#h_split_v = np.zeros([m_valid])
#for i in range(0,m_valid):
#    h_split_v[i] = h_sig[i]
#svm_accuracy_sig_valid = np.mean(h_split_v == y_valid)

#print('theta_sig')
#print(theta)
#print('accuracy_sig_train')
#print(svm_accuracy_sig_train)
#print('accuracy_sig_valid')
#print(svm_accuracy_sig_valid)

#tn_t = 0
#fn_t = 0
#tp_t = 0
#fp_t = 0
#for i in range(0,m_train):
#    if h_split_t[i] == 0 and y[i] == 0:
#        tn_t += 1
#    if h_split_t[i] == 0 and y[i] == 1:
#        fn_t += 1
#    if h_split_t[i] == 1 and y[i] == 0:
#        fp_t += 1
#    if h_split_t[i] == 1 and y[i] == 1:
#        tp_t += 1

#print('tn_t = ',tn_t)
#print('fn_t = ',fn_t)
#print('fp_t = ',fp_t)
#print('tp_t = ',tp_t)

#sig_precision_train = tp_t/(tp_t + fp_t)
#sig_recall_train = tp_t/(tp_t + fn_t)

#print('training precision')
#print(sig_precision_train)
#print('training recall')
#print(sig_recall_train)

#tn_v = 0
#fn_v = 0
#tp_v = 0
#fp_v = 0
#for i in range(0,m_valid):
#    if h_split_v[i] == 0 and y[i] == 0:
#        tn_v += 1
#    if h_split_v[i] == 0 and y[i] == 1:
#        fn_v += 1
#    if h_split_v[i] == 1 and y[i] == 0:
#        fp_v += 1
#    if h_split_v[i] == 1 and y[i] == 1:
#        tp_v += 1
#sig_precision_valid = tp_v/(tp_v + fp_v)
#sig_recall_valid = tp_v/(tp_v + fn_v)

#print('validation precision')
#print(sig_precision_valid)
#print('validation recall')
#print(sig_recall_valid)
